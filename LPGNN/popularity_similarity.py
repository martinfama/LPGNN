...
def drawPSNetwork(PS:pyg.data.Data, save_to=None, **kwargs):
    ...
    # if node_colors is passed, use it to color nodes
    if kwargs.get('node_color') is not None: node_color = kwargs.get('node_color') 
    elif hasattr(PS, 'node_polar_positions'): node_color = PS.node_polar_positions[:,1].detach().numpy() / (2*np.pi)
    else: node_color = 'cornflowerblue'

    node_size = [5*deg[1] for deg in PS_nx.degree]
    nx.draw(PS_nx, node_color=node_color, pos=pos, cmap='hsv',
                          node_size=node_size, width=0.2, with_labels=kwargs.get('with_labels', False), ax=ax)
