...
def drawPSNetwork(PS:pyg.data.Data, **kwargs):
    """ Function to plot a PS like network. More generally, it can accept a PyTorch-Geometric Data object
        ...
        can be either cartesian or polar coordinates. If no specific coordinate names are passed (such as those
        given by an embedding), the network is assumed to have 'node_polar_positions' as attribute, which is given
        when PS network are generated by `generatePSNetwork`.
